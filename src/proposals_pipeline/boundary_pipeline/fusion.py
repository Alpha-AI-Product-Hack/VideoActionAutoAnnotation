...
def fuse(per_source: dict[str, list[Proposal]], params: dict) -> list[Proposal]:
    ...
    source and video, multiplied by the source weight (0 drops the source),
    then greedily suppressed within `nms_window_s`; a kept proposal's fused
    score adds the best suppressed score of every other source, and
    proposals below `min_fused_score` are dropped."""

...
import einops
import copy
# vendoring note (action_boundaries): original import was
# `from torchvision.models.utils import load_state_dict_from_url`, a path
# torchvision removed years ago; torch.hub's own copy is the same function
# and drops the torchvision dependency entirely, which this repo doesn't
# otherwise need (see action_boundaries/gebd.py for the rest of the notes).
...
class resnetGEBD(nn.Module):
    ...
    def forward(self, x):
        ...
        # vendoring fix (action_boundaries): bare `.squeeze()` removes *every*
        # size-1 dim, including the batch dim whenever a caller's batch size
        # is 1 -- `(B*T, C, 1, 1)` -> `(C,)` instead of `(1, C)`, which then
        # breaks the `einops.rearrange(..., "(b f) c -> b f c", b=B)` below
        # and everything downstream. `.flatten(1)` always collapses exactly
        # the trailing (1, 1) spatial dims, regardless of B or C.
        x2 = self.avg_pool(x2).flatten(1)
        x3 = self.avg_pool(x3).flatten(1)
        x4 = self.avg_pool(x4).flatten(1)
        ...
        for t1 in range(T):
            ...
            # vendoring fix (action_boundaries): same bare-`.squeeze()`
            # batch-size-1 issue as above -- `(B, num_frames, 1)` must drop
            # only the trailing dim (`softmax(dim=1)` below needs `num_frames`
            # to still be axis 1, not collapsed away when B==1).

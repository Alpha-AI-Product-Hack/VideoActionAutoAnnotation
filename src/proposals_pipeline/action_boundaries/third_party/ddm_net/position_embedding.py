...
class PositionEmbeddingSine(nn.Module):
    def __init__(self, hidden_dim=512, num_locations=11):
        super().__init__()
    ...
    def forward(self, locations):
        # vendoring fix (action_boundaries): upstream slices dim 0 (the
        # singleton batch dim `position_table` was built with, via
        # `.unsqueeze(0)` in `_get_encoding_table`), which is a no-op --
        # `table[:N]` on a size-1 axis always returns that same 1 row-block,
        # so this always returned all `num_locations` positions regardless of
        # the requested sequence length. That's a real shape bug: at
        # frames_per_side=5 (this repo's only configuration -- T=10 input
        # frames), it returns 11 positions against 10 tokens and the later
        # `tensor + pos` add in transformer.py's `with_pos_embed` raises a
        # non-broadcastable shape error. Slicing dim 1 (the actual position
        # axis) is the evident intent.
        result = (
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "RTD-Net training and evaluation script", parents=[get_args_parser()]
    )
